  ...
  const buildScenario = (key, {allowPool = false} = {}) => {
    ...
    for (const edition of ["enterprise", "standard"]) {
      ...
      allocation.push(`${edition}: ${layout.topology === "pool"
        ? `${instancesPer[edition]} instance(s) of ${layout.instanceSize} vCore in ${sizeList.length} pool(s) of ${sizeList.length > 6 ? `${sizeList[0]} vCore each` : sizeList.join(" + ")}`
        : `${sizeList.length} ${instancesPer ? "instance" : "deployment"}(s) of ${sizeList.length > 6 ? `${sizeList[0]} ${key === "vm" ? "vCPU" : "vCore"} each` : sizeList.join(" + ")}`} for ${required[edition]} required; ${covered} ${key === "vm" ? "vCPU" : "vCore"} assumed SQL AHB${key === "vm" && input.ahb ? "; Windows Server AHB assumed" : ""}`);
    }
